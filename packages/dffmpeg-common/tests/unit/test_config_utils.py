...
def test_load_hmac_key_precedence(tmp_path):
    ...
    data = {"hmac_key": "direct_secret", "hmac_key_file": str(key_file)}
    key = load_hmac_key(data, Path("/tmp/config.yaml"))
    assert key == "file_secret"
...
def test_inject_transport_defaults():
    ...
    inject_transport_defaults(transports, coord, client_id="cid", hmac_key="key", default_poll_wait=10)
...
def test_inject_transport_defaults_preserve():
    transports = ClientTransportConfig(transport_settings={"http_polling": {"poll_wait": 20}})
    coord = CoordinatorConnectionConfig()

    inject_transport_defaults(transports, coord, client_id="cid", hmac_key="key")
